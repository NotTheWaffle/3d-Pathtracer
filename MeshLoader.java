...
/**
 * A static class used for the loading of Meshes
 * MeshLoader
 */
public class MeshLoader{
	public static final float EPSILON = PhysicalObject.EPSILON;
	private MeshLoader(){}
	/**
	 * Returns a Mesh from the specified equation. Assumes an ISO of 0. Utilizes marching cubes to triangulate the surface.
	 * @param equation
	 * @param bounds
	 * @param delta
	 * @param material
	 ...
	 * @return
	 */
	public static Mesh loadImplicitEquation(ImplicitEquation equation, AABB bounds, float delta, Material material, boolean normals) {
		List<Triangle> tris = new ArrayList<>();
		System.out.println("Loading a implicit Equation");
		for (float x = bounds.minX; x < bounds.maxX; x += delta) {
			for (float y = bounds.minY; y < bounds.maxY; y += delta) {
			outer:
				for (float z = bounds.minZ; z < bounds.maxZ; z += delta) {
					// cube corners
					Vec3[] p = new Vec3[8];
					p[0] = new Vec3(x,   y,   z  );
					p[1] = new Vec3(x+delta, y,   z  );
					p[2] = new Vec3(x,   y,   z+delta);
					p[3] = new Vec3(x+delta, y,   z+delta);
					p[4] = new Vec3(x,   y+delta, z  );
					p[5] = new Vec3(x+delta, y+delta, z  );
					p[6] = new Vec3(x,   y+delta, z+delta);
					p[7] = new Vec3(x+delta, y+delta, z+delta);

					float[] v = new float[8];
					for (int i = 0; i < 8; i++){
						v[i] = equation.apply(p[i].x, p[i].y, p[i].z);
						if (Float.isNaN(v[i])) continue outer;
					}
					...
					if ((edges & 1) != 0) verticies[0] = interp(p[0], p[1], v[0], v[1]);
					if ((edges & 2) != 0) verticies[1] = interp(p[1], p[3], v[1], v[3]);
					if ((edges & 4) != 0) verticies[2] = interp(p[3], p[2], v[3], v[2]);
					if ((edges & 8) != 0) verticies[3] = interp(p[2], p[0], v[2], v[0]);

					if ((edges & 16) != 0) verticies[4] = interp(p[4], p[5], v[4], v[5]);
					if ((edges & 32) != 0) verticies[5] = interp(p[5], p[7], v[5], v[7]);
					if ((edges & 64) != 0) verticies[6] = interp(p[7], p[6], v[7], v[6]);
					if ((edges & 128) != 0) verticies[7] = interp(p[6], p[4], v[6], v[4]);

					if ((edges & 256) != 0) verticies[8] = interp(p[0], p[4], v[0], v[4]);
					if ((edges & 512) != 0) verticies[9] = interp(p[1], p[5], v[1], v[5]);
					if ((edges & 1024) != 0) verticies[10] = interp(p[3], p[7], v[3], v[7]);
					if ((edges & 2048) != 0) verticies[11] = interp(p[2], p[6], v[2], v[6]);
					...
					for (int i = 0; i < triangles.length; i += 3){
						// wind one way vs the other
						Vec3 v1 = verticies[triangles[i]];
						Vec3 v2 = verticies[triangles[i+1]];
						Vec3 v3 = verticies[triangles[i+2]];
						Vec3 a = v2.sub(v1);
						Vec3 b = v3.sub(v1);

						float area = Vec3.ZERO_VEC.dist(a.cross(b));
						if (area < .00001) continue;

						Vec3 center = v1.add(v2).add(v3).mul(1f/3f);
						float h = .001f;
						Vec3 gradient = new Vec3(
							equation.apply(center.x+h, center.y, center.z) - equation.apply(center.x-h, center.y, center.z),
							equation.apply(center.x, center.y+h, center.z) - equation.apply(center.x, center.y-h, center.z),
							equation.apply(center.x, center.y, center.z+h) - equation.apply(center.x, center.y, center.z-h)
						);
						Triangle tri1 = new Triangle(v1, v2, v3 );
						if (tri1.normal().dot(gradient) > 0){
							tris.add(tri1);
						} else {
		...
		System.out.println("Loaded a implicit equation with "+tris.size()+" triangles");
		return loadObject(new ObjParser.ObjStruct(tris), new Transform(), 0, material, normals, true, true);
	}
	private static Vec3 interp(Vec3 p1, Vec3 p2, float v1, float v2) {
	...
	private static Mesh loadObject(ObjParser.ObjStruct objStruct, Transform transform, float size, Material material, boolean interpolatedNormals, boolean maintainAspectratio, boolean sahConstruction){
		...
		if (size == 0){
			...
			if (maintainAspectratio){
				...
				yscale = size/yrange;
				zscale = size/zrange;
				xrange = yrange = zrange = size;
			}
		}

		for (int i = 0; i < objStruct.vertices.size(); i++){
			Vec3 p = objStruct.vertices.get(i);
			if (p == null) continue;
			objStruct.vertices.set(i,
				new Vec3(
		...
		if (interpolatedNormals){
			List<Triangle> triangles = objStruct.getInterpolatedTriangles();
			return new Mesh(triangles, material, transform, sahConstruction);
		} else {
			List<Triangle> triangles = objStruct.getTriangles();
			return new Mesh(triangles, material, transform, sahConstruction);
		}
	}
}